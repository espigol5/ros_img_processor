...
void RosImgProcessorNode::process()
{
    ...
    if ( cv_img_ptr_in_ != nullptr )
    {
        ...
        cv_img_out_.image = cv_img_ptr_in_->image;
				...
				for(unsigned int ii = 0; ii < circles.size(); ii++ )
				{
				    if ( circles[ii][0] != -1 )
				    {
					    ...
					    cv::circle(cv_img_ptr_in_->image, center, 5, cv::Scalar(0,255,0), -1, 8, 0 );// circle center in green
					    cv::circle(cv_img_ptr_in_->image, center, radius, cv::Scalar(0,0,255), 3, 8, 0 );// circle perimeter in red
				    }
				}
		...
		Vector3d vecTransposed(3);
		vecTransposed = v.transpose();

		 Matrix3d matrixInverted;
		matrixInverted = matrixK_.inverse();

		direction_ = matrixInverted * vecTransposed;
